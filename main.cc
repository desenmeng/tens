...
napi_value GetAllOpList(napi_env env, napi_callback_info info)
{
  ...
  status = napi_create_buffer_copy(env, buffer->length, buffer->data, result_data, &result);
  return result;
}
...
void Init(napi_env env, napi_value exports, napi_value module, void *priv)
{
  ...
  napi_property_descriptor desc1 = {"sessionRun", 0, SessionRun, 0, 0, 0, napi_default, 0};
  status = napi_define_properties(env, exports, 1, &desc1);
  napi_property_descriptor desc2 = {"getAllOpList", 0, GetAllOpList, 0, 0, 0, napi_default, 0};
